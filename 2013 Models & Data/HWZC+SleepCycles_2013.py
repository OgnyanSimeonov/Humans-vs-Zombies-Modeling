...
#First time period from t=0h to t=60h. We keep b=0, thus it is not included in the 
...
def ode_model(z, t, k1, k2, b = 0): #define the ODE model
    ...
    if H > 0 and Z > 0: #System of ODEs to model the change in populations
        dHdt = -k1*0.5*(math.sin(t*math.pi/12 + 4.5)+1)*H*Z
        dWdt = -k2*0.5*(math.sin(t*math.pi/12 + 4.5)+1)*W*Z
        dZdt = k1*0.5*(math.sin(t*math.pi/12 + 4.5)+1)*H*Z + k2*0.5*(math.sin(t*math.pi/12 + 4.5)+1)*W*Z - b*Z
        dCdt = b*Z
    else:
        dHdt = 0
        dWdt = 0
...
initZ = 1
initC = 0
initial_conditions = [initH, initW, initZ, initC]
...
t0 = np.arange(1, 62, 1)
int_cond0 = [initH, initW, initZ, initC]
sol0 = odeint(ode_model, int_cond0, t0,args=(k1,k2))

plt.plot(t0,sol0[:,0]+sol0[:,1],'b-',label="Humans Fit")
plt.plot(t0,sol0[:,2],'g-',label="Zombies Fit")
plt.plot(t0,sol0[:,3],'r-',label="Corpses Fit")
...
def ode_model1(z, tspan1, k1, k2, b):
    ...
    #N = H + Z + C
    if H > 0 and Z > 0:
        dHdt = -k1*0.5*(math.sin(tspan1*math.pi/12 -3)+1)*H*Z
        dWdt = -k2*0.5*(math.sin(tspan1*math.pi/12 -3)+1)*W*Z
        dZdt = k1*0.5*(math.sin(tspan1*math.pi/12 -3)+1)*H*Z + k2*0.5*(math.sin(tspan1*math.pi/12 -3)+1)*W*Z - b*Z
        dCdt = b*Z
    else:
        dHdt = 0
        dWdt = 0
...
initZ1 =  sol0[days-2, 2]
initC1 = sol0[days-2, 3]
int_cond1 = [initH1, initW1 , initZ1, initC1]

b =  result1.params['b'].value
k1 = result1.params['k1'].value
k2 = result1.params['k2'].value
    
sol1 = odeint(ode_model1, int_cond1, t1, args=(k1,k2,b))


plt.plot(t1,sol1[:,0]+sol1[:,1],'b-')
plt.plot(t1,sol1[:,2],'g-')
plt.plot(t1,sol1[:,3],'r-')


#----------------------------------------------------------------------------------

plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
plt.savefig('TIMEW2013_Plot.png', bbox_inches='tight', dpi=300)